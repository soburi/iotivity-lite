...
void OCBlockwiseResponseState::set_etag(const Napi::CallbackInfo& info, const Napi::Value& value)
{
    for(uint32_t i=0; i<COAP_ETAG_LEN; i++) {
        m_pvalue->etag[i] = value.As<Napi::Buffer<uint8_t>>().Data()[i];
    }
}
...
{
    for(uint32_t i=0; i<value.As<Napi::Buffer<uint8_t>>().Length(); i++) {
        m_pvalue->buffer[i] = value.As<Napi::Buffer<uint8_t>>().Data()[i];
    }
}
...
void OCBlockwiseState::set_token(const Napi::CallbackInfo& info, const Napi::Value& value)
{
    for(uint32_t i=0; i<COAP_TOKEN_LEN; i++) {
        m_pvalue->token[i] = value.As<Napi::Buffer<uint8_t>>().Data()[i];
    }
}
#endif
...
void OCMemb::set_count(const Napi::CallbackInfo& info, const Napi::Value& value)
{
    for(uint32_t i=0; i<m_pvalue->num; i++) {
        m_pvalue->count[i] = value.As<Napi::Buffer<int8_t>>().Data()[i];
    }
}
...
{
    for(uint32_t i=0; i<value.As<Napi::Buffer<uint8_t>>().Length(); i++) {
        m_pvalue->data[i] = value.As<Napi::Buffer<uint8_t>>().Data()[i];
    }
}
...
Napi::Value OCRepresentation::get_byte_string_array(const Napi::CallbackInfo& info) {
    ...
    std::string key_ = info[0].As<Napi::String>().Utf8Value();
    const char* key = key_.c_str();
    OCStringArray& value = *OCStringArray::Unwrap(info[1].As<Napi::Object>());
    size_t* size = reinterpret_cast<size_t*>(info[2].As<Napi::Uint32Array>().Data());
...
Napi::Value OCRepresentation::to_json(const Napi::CallbackInfo& info) {
    OCRepresentation& rep = *OCRepresentation::Unwrap(info.This().As<Napi::Object>());
    char* buf = const_cast<char*>(info[0].As<Napi::String>().Utf8Value().c_str());
...
void OCUuid::set_id(const Napi::CallbackInfo& info, const Napi::Value& value)
{
    for(uint32_t i=0; i<16; i++) {
        m_pvalue->id[i] = info[0].As<Napi::Buffer<uint8_t>>().Data()[i];
    }
}
