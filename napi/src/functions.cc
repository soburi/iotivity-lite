...
Napi::Value N_coap_set_header_if_match(const Napi::CallbackInfo& info) {
  ...
  const uint8_t* etag = info[1].As<Napi::Buffer<const uint8_t>>().Data();
  size_t etag_len = static_cast<size_t>(info[2].As<Napi::Number>().Uint32Value());
  return Napi::Number::New(info.Env(), coap_set_header_if_match(packet, etag, etag_len));
}
#endif
...
Napi::Value N_coap_set_header_location_path(const Napi::CallbackInfo& info) {
  ...
  return Napi::Number::New(info.Env(), coap_set_header_location_path(packet, path));
}
#endif
...
Napi::Value N_coap_set_header_proxy_scheme(const Napi::CallbackInfo& info) {
  ...
  const char* scheme = scheme_.c_str();
  return Napi::Number::New(info.Env(), coap_set_header_proxy_scheme(packet, scheme));
}
#endif
...
Napi::Value N_coap_set_header_proxy_uri(const Napi::CallbackInfo& info) {
  ...
  return Napi::Number::New(info.Env(), coap_set_header_proxy_uri(packet, uri));
}
#endif
...
Napi::Value N_coap_set_header_uri_host(const Napi::CallbackInfo& info) {
  ...
  return Napi::Number::New(info.Env(), coap_set_header_uri_host(packet, host));
}
#endif
...
#if defined(XXX)
Napi::Value N_coap_get_observers(const Napi::CallbackInfo& info) {
  //func return unknown oc_list_t
}
#endif
...
Napi::Value N_oc_rep_clear_cbor_errno(const Napi::CallbackInfo& info) {
...
Napi::Value N_helper_rep_get_long(const Napi::CallbackInfo& info) {
  ...
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
// 2 jni_rep_get_error_flag, bool*
  return Napi::Number::New(info.Env(), 0);
}

Napi::Value N_helper_rep_get_bool(const Napi::CallbackInfo& info) {
  OCRepresentation& rep = *OCRepresentation::Unwrap(info[0].As<Napi::Object>());
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
// 2 jni_rep_get_error_flag, bool*
...
Napi::Value N_helper_rep_get_double(const Napi::CallbackInfo& info) {
  OCRepresentation& rep = *OCRepresentation::Unwrap(info[0].As<Napi::Object>());
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
// 2 jni_rep_get_error_flag, bool*
...
Napi::Value N_helper_rep_get_byte_string(const Napi::CallbackInfo& info) {
  OCRepresentation& rep = *OCRepresentation::Unwrap(info[0].As<Napi::Object>());
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
  size_t* byte_string_size = reinterpret_cast<size_t*>(info[2].As<Napi::Uint32Array>().Data());
  return Napi::String::New(info.Env(), helper_rep_get_byte_string(rep, key, byte_string_size));
}

Napi::Value N_helper_rep_get_string(const Napi::CallbackInfo& info) {
  OCRepresentation& rep = *OCRepresentation::Unwrap(info[0].As<Napi::Object>());
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
...
Napi::Value N_helper_rep_get_object(const Napi::CallbackInfo& info) {
  ...
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
  std::shared_ptr<oc_rep_t> sp(helper_rep_get_object(rep, key));
  auto args = Napi::External<std::shared_ptr<oc_rep_t>>::New(info.Env(), &sp);
  return OCRep::constructor.New({args});
}

Napi::Value N_helper_rep_get_object_array(const Napi::CallbackInfo& info) {
  OCRepresentation& rep = *OCRepresentation::Unwrap(info[0].As<Napi::Object>());
  std::string key_ = info[1].As<Napi::String>().Utf8Value();
  const char* key = key_.c_str();
  std::shared_ptr<oc_rep_t> sp(helper_rep_get_object_array(rep, key));
  auto args = Napi::External<std::shared_ptr<oc_rep_t>>::New(info.Env(), &sp);
  return OCRep::constructor.New({args});
}
